...
def try_decode_base64(data: str) -> str:
    ...
    Tự động thêm padding nếu cần thiết để decode thành công.
    """
    try:
        # 1. Kiểm tra ký tự (chỉ chứa A-Z, a-z, 0-9, +, /, =)
        ...
        # 2. Tự động thêm padding nếu cần thiết
        ...
        # 3. Clean up null bytes and control characters that break regex matching
        # Keep some control chars for attack payloads but remove null bytes
        decoded_str = decoded_str.replace('\x00', '')

        # 4. Kiểm tra xem kết quả có decode thành công (không chỉ garbage)
        # Nếu decode ra garbage text, trả về data gốc
        # Allow control chars for attack payloads
        if len(decoded_str) < 2 or all(ord(c) < 32 for c in decoded_str):
            return data
...
if __name__ == "__main__":
    ...
    example_2 = "PDxzY3IAaXB0L3NyYz1odHRwOi8veHNzLmNvbS94c3MuanM+PC9zY3JpcHQ"
